...
(defn make-server [port]
  (let [descriptors    (atom {})
        ...
        tcp-server     (delay (tcp/start-server acceptor message-buffer))
        updater-signal (stream) ;; TODO what properties does this stream need?
        updater        (partial update! message-buffer)
